...
class DataGeneration:
    ...
    def generate_random_financial_data(self, n_samples):
        ...
        # Number of accounts influenced by credit history
        ...
        data['n_accounts'] = np.random.poisson(base_accounts * credit_factor)

        # Number of loans with correlation to credit history
        zeros = np.random.binomial(1, 0.3 / (1 + 0.1 * credit_factor), n_samples)  # Less zeros for older accounts
        base_loans = stats.poisson.rvs(mu=1.5, size=n_samples)
        data['n_loans'] = base_loans * (1 - zeros)

        # Years in default
        data['years_since_default'] = stats.gamma.rvs(a=2, scale=2, size=n_samples)

        # LGD correlated with years in default
        default_factor = np.clip(data['years_since_default'] / 5, 0, 1)
        data['loss_given_default'] = stats.beta.rvs(10 * (1 + default_factor*0), 5, size=n_samples)

        # Default debt amount correlated with number of loans and LGD
        ...
        data['default_debt_amount'] = debt_base * (1 + 0.1 * loan_factor) * (1 + data['loss_given_default'])

        # Number of credit cards correlated with credit history
        ...
        # Number of refinances correlated with years in default
        ...
        data['months_since_first_payment'] = base_months * (1 + 0.0 * default_factor)

        # External debt correlated with default debt and number of loans
        rho = 0.1
        external_debt_base = stats.lognorm.rvs(s=0.8, scale=np.exp(9), size=n_samples)
        loan_debt_factor = np.clip(data['n_loans'] / 3, 0, 1)
        external_debt = (rho * (data['default_debt_amount'] / data['default_debt_amount'].std()) + 
                        (1 - rho) * (external_debt_base / external_debt_base.std())) * external_debt_base.std()
        # data['debt_cirbe'] = np.maximum(external_debt * (1 + 0.2 * loan_debt_factor), 0)
    ...
    def generate_treatment(self, df, noise=True,  noise_scale=0.2, treament_noise_std=5):
        ...
        linear_part = (
            ...
            0.4 * df_scaled['loss_given_default'] +
            0.2 * df_scaled['n_refin']
        )
        
        # Interactions and non-linear terms
        linear_part += (
            0.2 * df_scaled['years_since_default'] * df_scaled['default_debt_amount'] +
