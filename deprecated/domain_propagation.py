    ...
    def transform(self, X, y=None):
        ...
        if self.undersampling_perc_extrapolation:
            ...
            if self.stratified_subsampling:
                n_bins = 10  
                bins = pd.qcut(data[self.treatment], n_bins, duplicates='drop', labels=False)
        ...
        if self.interpolation_method:
            ...
            if self.undersampling_perc_interpolation:
                ...
                data_interpolation['combination_group'] = np.array(
                    data_interpolation
                    .reset_index()
                    .groupby(["index", self.treatment])
                    .ngroup()
                )
                ...
                if self.stratified_subsampling:
                    ...
                    combination_treatment_means = data_interpolation.groupby('combination_group')[self.treatment].mean()
                    combination_bins = pd.qcut(combination_treatment_means, len(ref_proportions), labels=False)
                    
                    target_combinations = int(len(combination_treatment_means) * (1 - self.undersampling_perc_interpolation))
                    bin_samples = (ref_proportions * target_combinations).round().astype(int)
                    
                    # Sample combination groups from each bin
                    sampled_combinations = []
                    for bin_label, n in bin_samples.items():
                        bin_combinations = combination_treatment_means[combination_bins == bin_label].index
                        if len(bin_combinations) > 0:
                            sampled_combinations.extend(
                                np.random.choice(
                                    bin_combinations,
                                    size=min(n, len(bin_combinations)),
                                    replace=False
                                )
                            )
                    data_interpolation = data_interpolation[
                        data_interpolation['combination_group'].isin(sampled_combinations)
                    ].drop('combination_group', axis=1)
                    
                else:
                    unique_indices_treatment = data_interpolation['combination_group'].unique()
                    cases_to_remove = np.random.choice(
                        unique_indices_treatment, 
                        size=int(len(unique_indices_treatment) * self.undersampling_perc_interpolation), 
                        replace=False
                    )
                    data_interpolation = data_interpolation[
                        ~data_interpolation['combination_group'].isin(cases_to_remove)
                    ].drop('combination_group', axis=1)
